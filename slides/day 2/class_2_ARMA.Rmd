...
1. Base your future predictions on previous values of the data (AR models)
1. __Base your future predictions on how wrong you were in your past predictions__ (moving average models)


AR models are based on previous values

MA models are based on previous forecast error


If PACF has exponential decay and ACF has sticking out errors, you should use MA model

If ACF has exponential decay and PACF has sticking out errors, you should use AR model
...
- $\beta$ is usually constrained (naturally via the data) to lie between -1 and 1 (usually between 0 and 1). Outside that range the process blows up
- The sample PACF is often a good way of diagnosing if an AR model might be appropriate
  - if PACF decays exponentially and is mostly small, good indication to use AR model with order being number of lags at which PACF becomes small

## Intro to Moving Average Models

- Moving Average (MA) models are similar to AR models but they depend on the previous residual of the series rather than the value itself (moving average refers to previous forecast error)
...
- ARMA models can deal with a very wide variety of flexible time series behaviour, though they remain stationary (constant mean, variance and correlation structure)
...
forecast 20 lags into the future
...
- The answer is that we have fitted a _stationary model_, i.e. one with constant mean and variance - the data does not have a constant mean
